...
```{r}
rec <- rec %>% step_zv(all_predictors()) %>% step_nzv(all_predictors())
rec
prep(rec)
```
...
bake(prep(rec), diamonds %>% mutate(coluna_constante = 1))
# Queremos evitar o "data leakage" (vazamento de informação) entre base de treino e base de teste...
```
...
```{r}
rec2 <- rec %>% step_dummy(all_nominal())  # "Engines" que precisam de dummificação:
# glmnet, xgboost, ranger

juice(prep(rec2)) %>% glimpse()  # Equivalente a escrever...
bake(prep(rec2), new_data = NULL) %>% glimpse()
```
...
diamonds %>% 
  sample_n(1000) %>%
  filter(!is.na(x), x > 0, !is.na(price), price > 0) %>% 
  mutate(
    pred_lm = predict(lm(price ~ x, data = .))
  ) %>% 
  ggplot(aes(x = x)) +
  geom_point(aes(y = price)) +
    geom_line(aes( y = pred_lm, color = "red"), size = 1L)
  # + scale_y_log10()

# sobre modelos lineares ----->
# sobre problemas numericos
...
rec3 <- rec %>% step_poly(x, degree = tune())  # novidade...
juice(prep(rec3)) %>% glimpse()
...
```{r}
step_pca(threshold = 0.9)  # Gera componentes principais até X% da variância...
step_ica()
step_isomap()
step_umap()
step_kpca()
step_kpca_poly()
step_kpca_rbf()
step_interact() # modelos lineares, para criar interações...
```
...
# com o -all_outcomes()
rec3 <- rec %>% 
  step_pca(all_numeric(), -all_outcomes(), threshold = 0.999) 
juice(prep(rec3)) %>% glimpse()
```
